...
//	private double tpmC;
	private static java.util.concurrent.atomic.AtomicInteger seqno=new java.util.concurrent.atomic.AtomicInteger();
	public static String iWarehouses, iTerminals;
	...
	private String getProp(Properties p, String pName) {
		//优先从System Properties读取（命令行-D指定优先）
		
		String prop = System.getProperty(pName);
		if(prop==null){
			prop=p.getProperty(pName);
		}
	...
	public jTPCC() {
		...
		log.info("Term-00, ");
		iWarehouses = getProp(ini, "warehouses");
		iTerminals = getProp(ini, "terminals");
		verbose = (getProp(ini,"verbose")!=null);
		csp = (getProp(ini,"csp")!=null);
				...
				try {
					...
					for (int i = 0; i < numTerminals; i++) {
						...
						jTPCCTerminal terminal = new jTPCCTerminal(
								...
								terminalDistrictID, conn,
								transactionsPerTerminal, paymentWeightValue,
								orderStatusWeightValue, deliveryWeightValue,
					...
					errorMessage("This session ended with errors: "+e1.getMessage());
...
//			double tpmC = (6000000 * fastNewOrderCounter / (currTimeMillis - sessionStartTimestamp)) / 100.0;
			double tpmTotal = (6000000 * transactionCount / (currTimeMillis - sessionStartTimestamp)) / 100.0;
			informativeText.append("Term-00, Running Average tpmTOTAL: "
					+ tpmTotal + "    ");
		}
		...
		long freeMem = Runtime.getRuntime().freeMemory() / (1024 * 1024);
		long totalMem = Runtime.getRuntime().totalMemory() / (1024 * 1024);
		informativeText.append("Memory Usage: " + (totalMem - freeMem)
				+ "MB / " + totalMem + "MB");
